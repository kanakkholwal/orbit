import { getFontForLanguage } from '$utils/font-loader';
import { getPDFDocument } from '$utils/helper';
import { calculateSpaceTransform, calculateWordTransform, parseHocrDocument } from '$utils/hocr-transform';
import fontkit from '@pdf-lib/fontkit';
import { PDFDocument, PDFFont, StandardFonts, rgb } from 'pdf-lib';
import type { PDFDocumentProxy } from 'pdfjs-dist';
import Tesseract from 'tesseract.js';
import type { OcrLine, OcrPage } from './ocr-pdf-type';
...
function drawOcrTextLayer(
  ...
  ocrPage.lines.forEach((line: OcrLine) => {
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      try {
        page.drawText(text, {
          ...
          color: rgb(0, 0, 0),
          opacity: 0, // Crucial: Makes the text invisible but selectable
        });
      } catch (error) {
        console.warn(`Could not draw text "${text}":`, error);
      }

      // Inject spaces between words based on bounding box gaps
...
export async function performOcr(
  ...
  const pdf = (await (await getPDFDocument({ data: pdfBytes })).promise) as PDFDocumentProxy;
  ...
  try {
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      if (data.hocr) {
        ...
        drawOcrTextLayer(newPage, ocrPage, viewport.height, primaryFont, latinFont);
      }
