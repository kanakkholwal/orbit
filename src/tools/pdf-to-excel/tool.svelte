<script lang="ts">
  ...
  const tableCount = $derived(store.doneFiles.reduce((sum, f) => sum + (f.tables ?? 0), 0));
...
{#if store.files.length === 0}
  <UploadArea accept=".pdf,application/pdf" onFilesSelected={(files) => store.addFiles(files)}>
    {#snippet title()}
      <h3 class="text-heading-sm font-medium text-foreground">Drop PDFs to copy tables into Excel</h3>
    {/snippet}
    {#snippet description()}
      <p class="max-w-sm text-pretty text-body text-muted-foreground">
        Find the tables in each PDF and save them as a spreadsheet, one sheet per table.
      </p>
    {/snippet}
  </UploadArea>
{:else}
  <div class="flex flex-col gap-4">
    {#if showResult}
      <ResultCard
        title={`Found ${plural(tableCount, "table")}`}
        description={doneCount === 1
          ? "The spreadsheet is downloaded."
          : `${doneCount} spreadsheets are downloaded as one ZIP.`}
      >
        {#snippet actions()}
    ...
    <p class="text-body text-muted-foreground">
      Works best on tables with clear rows and columns in PDFs made on a computer.
    </p>
      ...
      {#each store.files as file (file.id)}
        <li>
          ...
          >
            ...
            {#if file.status === "done" && file.tables}
              <span>· {plural(file.tables, "table")}</span>
            {/if}

            {#snippet trailing()}
              {#if file.status === "processing"}
                <StatusPill status="processing" label="Finding tables" />
              {:else if file.status === "done"}
                ...
                <Button
                  ...
                  aria-label={`Download spreadsheet for ${file.file.name}`}
  ...
  <ToolFooter>
    {#snippet hint()}
      ...
      {:else}
        <span class="block truncate">All files checked for tables</span>
      {/if}
    {/snippet}

    <Button variant="primary" onclick={() => store.process()} disabled={store.isProcessing || pendingCount === 0}>
      {store.isProcessing ? "Converting…" : pendingCount > 0 ? `Convert ${plural(pendingCount)}` : "Convert"}
    </Button>
  </ToolFooter>
{/if}
