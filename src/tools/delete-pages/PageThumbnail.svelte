<script lang="ts">
  ...
  let { store, index }: { store: DeletePagesState; index: number } = $props();

  let canvas: HTMLCanvasElement;
  let isRendered = $state(false);

  const selected = $derived(store.state.pagesToDelete.has(index));
...
<button
  ...
  aria-label={`Page ${pageNumber}${selected ? ", will be deleted" : ""}`}
...
>
  ...
  <span class="flex min-h-5 items-center justify-between gap-1 px-0.5 text-caption tabular-nums" aria-hidden="true">
    ...
    {#if selected}
      <span class="shrink-0 font-medium text-foreground">Delete</span>
    {/if}
  </span>
</button>
