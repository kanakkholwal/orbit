      ...
      (entries) => {
        if (entries[0].isIntersecting) {
          if (canvasEl && !isRendered) {
            store
              .renderThumbnail(canvasEl, pageInfo.index)
...
<div
  ...
  onkeydown={(e) =>
    (e.key === "Enter" || e.key === " ") &&
    store.togglePageSelection(pageInfo.index)}
>
  <div
    class="relative overflow-hidden rounded-xl border-2 aspect-3/4 flex items-center justify-center bg-muted/30 shadow-sm transition-all duration-200
           {pageInfo.isSelected
      ? 'border-destructive ring-2 ring-destructive/20 opacity-90'
  ...
  >
    ...
    <div
      class="absolute bottom-0 left-0 right-0 bg-background/80 backdrop-blur-sm border-t border-border text-center py-1 text-xs font-medium"
    >
