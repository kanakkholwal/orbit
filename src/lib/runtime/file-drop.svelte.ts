 ...
 * paths from disk, rebuilds `File` objects, and hands them to the page's primary
 * intake: the earliest registered handler, so a secondary field never steals a PDF.
 */
		...
		const unlisten = await getCurrentWebview().onDragDropEvent(
			async (event) => {
				...
				if (payload.type === "enter" || payload.type === "over") {
					...
					if (files.length) handlers[0](files);
				}
			},
		);
		return () => {
