...
<div class="relative flex min-h-screen w-full flex-col">
  ...
  <main class="mx-auto w-full max-w-3xl px-5 pb-24 pt-32 md:px-8 md:pt-40">
    <header class="flex flex-col gap-5 border-b border-border pb-10">
    ...
    <div class="flex flex-col gap-12 py-12">
      {#each sections as section (section.heading)}
        <section class="flex flex-col gap-4">
          <h2 class="text-subheading text-foreground">{section.heading}</h2>
          {#if section.paragraphs}
            {#each section.paragraphs as p (p)}
              <p class="text-body leading-relaxed text-muted-foreground">{p}</p>
            {/each}
          {/if}
          {#if section.list}
